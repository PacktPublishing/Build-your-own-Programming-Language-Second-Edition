...
void check_block() {
  ...
  void calctype() {
	...
	case parser.IDENTIFIER:
	    ...
	    return;
	default:
	  j0.semerror("can't grok the type of " + tok.text + " cat " + tok.cat);
	}
	break;
  ...
  switch (sym) {
    ...
    case "FormalParmList":
      ...
      System.arraycopy(ta1, 0, ta, 0, ta1.length);
      System.arraycopy(ta2, 0, ta, ta1.length, ta2.length);
      return ta;
      }
    return null;
  }
  ...
  void checktype(boolean in_codeblock) {
      ...
      if (rule == 1290) {
        ...
        if (kids[0].sym.equals("QualifiedName")) {
  	    ...
  	    if (! (rve.typ instanceof methodtype))
              j0.semerror("method expected, got " + rve.typ.str());
	    rv = (methodtype)rve.typ;
            cksig(rv);
            }
          }
          else j0.semerror("can't typecheck token " + kids[0].tok.cat);
          }
        }
	...
	if (kids[0].typ.str().startsWith("array ")) {
	    ...
	    else j0.semerror("subscripting array with "+kids[1].typ.str());
          }
	else j0.semerror("illegal subscript on type "+ kids[0].typ.str());
	break;
    case "ReturnStmt":
      symtab_entry ste;

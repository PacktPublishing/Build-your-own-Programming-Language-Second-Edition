...
class tree {
  ...
  typeinfo typ;
  ArrayList<tac> icode;
  ArrayList<String> icncode, staticcode, vardecls;
...
void genunicon() {
  if (kids != null) for(tree k : kids) k.genunicon();
  switch (sym) {
  case "ClassDecl": { genuniClassDecl(); break; }		// XXX
  case "CondAndExpr": case "CondOrExpr": case "MulExpr":	// XXX
  case "AddExpr": { genuniBinaryExpr(); break; }		// XXX
  case "RelExpr": { genuniRelExpr(); break; }			// XXX
  case "EqExpr": { genuniEqExpr(); break; } 			// XXX
  case "WhileStmt": { genuniWhileStmt(); break; }		// ...
  case "IfThenStmt": { genuniIfThenStmt(); break; }		// XXX
  case "IfThenElseStmt": { genuniIfThenElseStmt(); break; }	// ...
  case "Assignment": { genuniAssignment(); break; }	  	// XXX
  case "MethodCall": { genuniMethodCall(); break; }		// XXX
  case "ArgList":    { genuniArgList(); break; }  		// XXX
  case "MethodDecl": { genuniMethodDecl(); break; }		// XXX
  case "QualifiedName": { genuniQualifiedName(); break; }	// XXX
  case "VarDeclarator": { genuniVarDeclarator(); break; }
  // ...
  case "FormalParm": { genuniFormalParm(); break; }
  case "MethodDeclarator": { genuniMethodDeclarator(); break; }
  case "MethodHeader": { genuniMethodHeader(); break; }
  case "Block": { genuniBlock(); break; }
  case "BlockStmts": { genuniBlockStmts(); break; }
  case "ClassBody": { genuniClassBody(); break; }
  case "LocalVarDecl": { genuniLocalVarDecl(); break; }
  case "token":   { genunitoken(); break; }			// XXX
  default: {
       icncode = new ArrayList<String>();
       if (kids != null) for(tree k:kids)
          icncode.addAll(k.icncode);
       }
   }
}
...
void genunitoken() {
  icncode = new ArrayList<String>();
  icncode.add( tok.text );
  switch (tok.cat) {
    case parser.IDENTIFIER: { break; }
    case parser.INTLIT: { break; }
    }
}

// discard type, keep name
void genuniFormalParm() {
  icncode = kids[1].icncode;
}

void genuniMethodDeclarator() {
 icncode = new ArrayList<String>();
  if (kids[1] != null)
    icncode.add(kids[1].icncode.get(0));
}

void genuniVarDeclarator() {
  icncode = kids[0].icncode;
}

void genuniArgList() {
  icncode = new ArrayList<String>();
  icncode.addAll(kids[0].icncode);
  icncode.add(",");
  icncode.addAll(kids[1].icncode);
}

void genuniMethodHeader() {
  String procname = kids[1].kids[0].icncode.get(0);
  String resword;
  if (rule == 1070) resword = "procedure ";
  else resword = "method ";
  icncode = new ArrayList<String>();
  icncode.add( resword + procname + "(" + kids[1].icncode.get(0) + ")" );
}

void genuniQualifiedName() {
  if (kids[1].icncode.get(0).equals("println") &&
      kids[0].kids[1].icncode.get(0).equals("out") &&
      kids[0].kids[0].sym.equals("token") &&
      kids[0].kids[0].icncode.get(0).equals("System")) {
    icncode = new ArrayList<String>();
    icncode.add("write");
    }
  else {
    switch (kids[0].typ.basetype) {
    case "array": {
      if (kids[1].tok.text.equals("length")) {
        icncode = new ArrayList<String>();
        icncode.add( "*" + kids[0].icncode.get(0) );
        }
      else {
        System.err.println("don't know how to generate code for array " +
                            kids[1].tok.text);
        System.exit(1);
        }
	break;
    ...
    case "class": {
      icncode = new ArrayList<String>();
      icncode.add( kids[0].icncode.get(0) + "." + kids[1].icncode.get(0) );
      break;
      }
    default: {
      System.err.println("don't know how to generate code for type " +
                            kids[0].typ.str());
      }
    }
  }
...
void genuniMethodCall() {
  icncode = new ArrayList<String>();
  if (rule==1290) {
    icncode.addAll( kids[0].icncode );
    String lastone = icncode.get(icncode.size()-1);
    icncode.remove(icncode.size()-1);
    icncode.add( lastone + "(" );
    icncode.addAll( kids[1].icncode);
    icncode.add( ")" );
    }
    ...
    icncode.addAll( kids[0].icncode );
    icncode.add( kids[1].tok.text + "(" );
    icncode.addAll( kids[2].icncode );
    icncode.add( ")" );
    }
}
...
void genuniBlock() {
  ...
  for (tree k : kids) {
    if (k.vardecls != null)
      icncode.addAll( k.vardecls ); // move decls up front
    icncode.addAll(k.icncode);
    }
  icncode.add( "}" );
}
...
void genuniMethodDecl() {
  staticcode = new ArrayList<String>();
  icncode = new ArrayList<String>();
  if (kids[0].rule == 1070) { // production rule # for public static
    staticcode.addAll(kids[0].icncode);
    for(int i = 1; i < kids[1].icncode.size()-1; i++)
      staticcode.add( kids[1].icncode.get(i) );
    staticcode.add( "end" );
    }
  else {
    for(tree k : kids)
...
void genuniClassBody() {
  staticcode = new ArrayList<String>();
  icncode = new ArrayList<String>();
  for (tree k : kids) {
    staticcode.addAll( k.staticcode );
    icncode.addAll( k.icncode );
    }
  icncode.add( "end" );
}

void genuniClassBodyDecls() {
  staticcode = new ArrayList<String>();
  icncode = new ArrayList<String>();
...
// generate public static methods outside the class, then generate the class
void genuniClassDecl() {
   icncode = new ArrayList<String>();
   icncode.add("package " + kids[0].icncode.get(0));
   icncode.addAll(kids[kids.length-1].staticcode);
   icncode.add("class " + kids[0].icncode.get(0) + "()");
   icncode.addAll(kids[kids.length-1].icncode);
}

void genuniRelExpr() {
  String op = kids[1].tok.text;
  icncode = new ArrayList<String>();
  icncode.add( kids[0].icncode.get(0) + " " + op + " " + kids[2].icncode.get(0) );
}

void genuniEqExpr() {
   String op;
   if (rule == 1340) op = "==="; else op = "~===";
   icncode = new ArrayList<String>();
   icncode.add(kids[0].icncode.get(0) + op + kids[1].icncode.get(0));
}

void genuniWhileStmt() {
   icncode = new ArrayList<String>();
   if (kids[1].icncode.get(0).equals("{")) {
      icncode.add("while " + kids[0].icncode.get(0) + " do {");
      for(int i=1; i<icncode.size(); i++)
        icncode.add( kids[1].icncode.get(i) );
      }
    else {
      icncode.add("while " + kids[0].icncode.get(0) + " do");
      icncode.addAll( kids[1].icncode );
...
void genuniIfThenStmt() {
   icncode = new ArrayList<String>();
   if (kids[1].icncode.get(0).equals("{")) {
     icncode.add("if " + kids[0].icncode.get(0) + " then {");
     for(int i = 1; i < kids[1].icncode.size(); i++)
       icncode.add( kids[1].icncode.get(i) );
     }
   else {
     icncode.add( "if " + kids[0].icncode.get(0) + " then");
...
void genuniLocalVarDecl() {
   vardecls = new ArrayList<String>();
   vardecls.add( "local " );
   vardecls.addAll( kids[1].icncode );
   icncode = new ArrayList<String>();
}

void genuniAssignment() {
  String op = kids[1].icncode.get(0);
  if (op.equals("=")) op = ":=";
...
void genuniBinaryExpr() {
  String op;
  switch (rule) {
  case 1320:
    if (typ.str().equals("String")) op = "||";
    else op = "+";
    break;
  case 1321: op = "-"; break;
  case 1310: op = "*"; break;
...
void genMethodDecl() {
    icode = new ArrayList<tac>();
    icode.addAll(gen("proc", new address(kids[0].kids[1].kids[0].tok.text, 0)));
...
void gentoken() {
  ...
  switch (tok.cat) {
    case parser.IDENTIFIER: {
      symtab_entry ste;
      //      System.out.println("gentoken on " + tok.text);
      ste = stab.lookup(tok.text);
      if (ste == null) {
	  //  System.out.println("ste null");
      }
      else {
	  // System.out.println("ste OK");
	  addr = ste.addr;
      }
      break;
      }

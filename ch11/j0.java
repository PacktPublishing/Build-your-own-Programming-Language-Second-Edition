...
public class j0 {
   ...
   public static ch11_2e.parser par;
   public static ch11_2e.symtab global_st, stringtab;
   ...
   public static void main(String argv[]) throws Exception {
      ...
      par = new parser();
      //                  par.yydebug=true;
      System.out.println("Transpiling " + yyfilename + " to .icn format");
      yylineno = yycolno = 1;
      int i = par.yyparse();
      if (i == 0)
  ...
  public static void semantic(parserVal r) {
    ...
    ch11_2e.symtab out_st, System_st;
    global_st = new ch11_2e.symtab("global");
    stringtab = new ch11_2e.symtab("strings");
    System_st = new ch11_2e.symtab("class");
    out_st = new ch11_2e.symtab("class");
  ...
  public static void genunicon(parserVal r) {
    tree root = (tree)(r.obj);
    root.genunicon();
  int len = yyfilename.length();
  String basenam = yyfilename.substring(0,yyfilename.length()-5);
  String outfileh = basenam + "_helper.icn";
  PrintWriter fout;
  try {
    fout = new PrintWriter(new BufferedWriter(new FileWriter(outfileh)));
    fout.printf("procedure main(argv)\n");
    fout.printf("   " + basenam + "__main ! argv\n");
    fout.printf("end\n");
    fout.close();
    }
  catch (java.io.IOException ioException) {
    System.err.println("can’t write "+outfileh);
    System.exit(1);
    }
  String outfilename = basenam + ".icn";
  try {
    fout = new PrintWriter(new BufferedWriter(new FileWriter(outfilename)));
    for(int i=0; i<root.icncode.size(); i++) {
      String s = root.icncode.get(i);
      fout.printf(s + "\n");
      System.out.println(s);
      }
    fout.close();

    String cmd = "unicon -s " + outfilename + " " + outfileh;
    ...
    System.out.println("unicon exited with status " + String.valueOf(eValue));
    }
  catch (java.io.IOException ioException) {
    System.err.println("can’t write "+outfilename);
    System.exit(1);
    }
}

...
class tree {
  ...
  public void print(int level) {
    ...
    for (tree k : kids) k.print(level+1);
  }
  public void print() {
    print(0);
  }
  ...
  void mkSymTables(symtab curr) {
   ...
   for (tree k : kids) k.mkSymTables(curr);
  }
  ...
  void calc_isConst() {
   ...
   for(tree k : kids) k.calc_isConst();
  }
...
void check_block() {
  ...
  case "MethodCall": {
   ...
   default:
      for (tree k : kids) k.check_block();
   }
  }

  typeinfo calctype() {
    for (tree k : kids) k.calctype();
    switch (sym) {
	  ...
	  case parser.INT: { return typ=new typeinfo(tok.text); }
          default: j0.semerror("don't know the type of " + tok.text);
	  }
      }
      default: j0.semerror("don't know the type of " + sym);
    }
    return null;
  }
